...
def model_select(args):
    ...
    if args.usenet == "bn_alexnet":
        ...
        weight_name = os.path.join(
            args.path2weight,
            args.dataset + "_" + args.usenet + "_epoch" + str(args.useepoch) + ".pth",
        )
        ...
        # ImageNet pre-trained model
            ...
            assert os.path.isfile(weight_name), "don't exists weight: {}".format(
                weight_name
            )
            ...
            model.classifier = nn.Sequential(
                *(list(model.classifier.children())[:-1] + list(last_layer))
            )
        removed = list(model.classifier.children())[:-1]
        model.classifier = torch.nn.Sequential(*removed)
        model.classifier = torch.nn.Sequential(
            model.classifier, nn.Linear(4096, args.numof_classes)
        )
