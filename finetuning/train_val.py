...
def train(args, model, device, train_loader, optimizer, epoch, iteration):
    ...
    for i_batch, sample_batched in enumerate(train_loader):
        data, target = sample_batched["image"].to(device), sample_batched["label"].to(
            device
        )
        optimizer.zero_grad()
        output = model(data)
        ...
        if i_batch % args.log_interval == 0:
            sys.stdout.write(
                "\repoch:{0:>3} iteration:{1:>6} train_loss: {2:.6f} train_accracy: {3:5.2f}%".format(
                    epoch,
                    iteration,
                    loss.item(),
                    100.0 * correct / float(len(sample_batched["label"])),
                )
            )
            sys.stdout.flush()
        iteration += 1
